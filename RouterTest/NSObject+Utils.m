...
+(NSString *)requestIdentifierWithSEL:(SEL)selector{
    NSString * workshopStr  = NSStringFromClass([Workshop class]);
    NSString * classStr = NSStringFromClass(self);
    NSString * selectorStr = NSStringFromSelector(selector);
    NSString * identifier = [[NSString stringWithFormat:@"%@+%@_%@",workshopStr,classStr,selectorStr] lowercaseString];
    NSLog(@"%@",identifier);
    return identifier;
}
...
-(void)throwExceptionWithName:(NSString *)name reason:(NSString *)reason userInfo:(NSDictionary *)userInfo{
    [[self class] throwExceptionWithName:name reason:reason userInfo:userInfo];
}
...
-(id)performSelector:(SEL)selector withParamsArray:(NSArray*)params{
    ...
    for (int i = 0; i<count; i++) {
        ...
        //objc_msgSend 会自动添加两个参数（id、SEL）   http://www.tuicool.com/articles/u2yeEnv
...
-(id)performSelector:(SEL)selector withParams:(id)firstParam, ...{
    ...
    return [self performSelector:selector withParamsArray:params];
}
